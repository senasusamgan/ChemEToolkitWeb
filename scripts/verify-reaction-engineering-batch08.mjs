...
const liveEntries = entries.filter((entry) => entry[4] === 'true')

if (entries.length !== 380 || liveEntries.length !== 376) {
  console.error(`Catalog mismatch: ${entries.length} total / ${liveEntries.length} live`)
  process.exit(1)
}
...
const categoryLive = liveEntries.filter(
  (entry) => entry[3] === 'Reaction Engineering',
).length

if (categoryLive !== 58) {
  console.error(`Reaction Engineering live count is ${categoryLive}, expected 58.`)
  process.exit(1)
}
...
if (
  !block ||
  !/\blive:\s*58\b/.test(block[0]) ||
  !/\btotal:\s*62\b/.test(block[0])
) {
  console.error('Reaction Engineering category is not 58 / 62.')
  process.exit(1)
}
...
console.log('Reaction Engineering Batch 08 verification PASS')
console.log('380 total / 376 live / 4 queued')
console.log('Reaction Engineering: 58 live / 62 total')
