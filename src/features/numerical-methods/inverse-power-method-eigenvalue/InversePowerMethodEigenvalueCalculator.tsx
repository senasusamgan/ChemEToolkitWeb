...
import type { InversePowerMethodEigenvalueResult } from './types'
import {
  ActionBar,
  CalculatorHeader,
  NumericInput,
...
export function InversePowerMethodEigenvalueCalculator() {
  ...
  const [shift, setShift] = useState(example.shift)
  ...
  const [result, setResult] =
    useState<InversePowerMethodEigenvalueResult | null>(null)
  const [errorMessage, setErrorMessage] = useState('')

  function calculate() {
    try {
      setResult(calculateInversePowerMethodEigenvalue({
  ...
  return (
    <section className="native-calculator">
      <CalculatorHeader
        ...
        subtitle="Shifted inverse iteration for an eigenvalue nearest a chosen shift"
      />

      <ReferenceBasis>
        (A − μI)zₖ₊₁ = vₖ followed by normalization
      </ReferenceBasis>
      ...
      <div className="native-input-grid">
        ...
        <NumericInput label="Eigenvalue Shift" symbol="μ" value={shift} unit="—" onChange={setShift} />
      ...
      <ActionBar onLoadExample={loadExample} onClear={clearInputs} onCalculate={calculate} calculateLabel="Run inverse power method" />

      {errorMessage ? <div className="native-error" role="alert">{errorMessage}</div> : null}

      {result ? (
        <ResultPanel
          headlineLabel="Eigenvalue"
        ...
        >
          ...
          <ResultItem label="Shifted Inverse Estimate" value={formatEngineeringNumber(result.shiftedInverseEstimate)} unit="—" />
        </ResultPanel>
      ) : null}
    </section>
  )
